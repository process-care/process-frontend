...
export const ProjectMenu: React.FC<Props> = ({ isOpen, surveyId, onClose }) => {
  ...
  const { title, description, date, stepsLeft, stats, exportURL } = useSurveyData(surveyId);
  ...
  return (
    ...
    <Container variant="rightPart" w="50%">
      <Box p={1} d="flex" w="100%" justifyContent="space-between">
        <Tooltip label="Fermer"><Button onClick={onClose}>🚪</Button></Tooltip>
        <Box>
          <Tooltip label="Editer"><Button onClick={nyi}>✏️</Button></Tooltip>
          <Tooltip label="Supprimer"><Button onClick={nyi}>☠️</Button></Tooltip>
        </Box>
      </Box>
      ...
      <Box p={5} textAlign="left">
        ...
        <a href={exportURL} download>
          <Button variant="roundedTransparent" size="xs" p={2}>CSV ⇣</Button>
        </a>
      </Box>
      
      <Box mt={4}>
        <Flex >
          <ActionButton top right label={"Voir la page d'accueil"} onClick={goToLanding}/>
          <ActionButton top right label={"Voir le questionnaire"} onClick={goToForm}/>
          <ActionButton top label={"Voir le consentement"} onClick={goToConsent}/>
        </Flex>
        <Flex>
          <ActionButton top right bottom label={"Dupliquer"} onClick={nyi}/>
          <ActionButton top right bottom label={"Archiver"} onClick={nyi}/>
          <ActionButton top bottom label={"Archiver & Dupliquer"} onClick={nyi}/>
        </Flex>
      </Box>
      ...
      <Box mt={10}>
        ...
        <Flex mt={55} ml={50} mr={50} flexDirection="column" alignItems="flex-start">
          <div>Progression de la complétion</div>
          <Tooltip label={"Bibou !"}><div>📈</div></Tooltip>
        </Flex>
      </Box>
    </Container>
  );
...
function useSurveyData(surveyId: string) {
  ...
  const stats: Record<string, {visits: number, consented: number, completed: number}> = {
...
const ActionButton = ({ top, right, bottom, left, label, onClick }: ActionButtonProps) => {
  const borders = {
  ...
  return (
    <Box p={3} w="calc(100% / 3)" {...borders} _hover={{ cursor: "pointer" }} onClick={onClick}>
...
function nyi() { alert(nyiMsg[Math.floor(Math.random() * nyiMsg.length)]); }
